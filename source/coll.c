...
void stackPush(Stack *stack,Image *image){
    ...
    stack->stackData[(stack->base + stack->tot) % (IMAGE_STACK_MAX_SIZE + 1) + 1] = imageData;
    sprintf(t,"%d",stack->tot);
    strcat(t,".tmp");
    strcat(dest,t);
    fp1 = fopen(s,"rb");
    fp2 = fopen(dest,"wb");
...
int stackPop(Stack *stack,Image *image){
    ...
    width = image->width;
    height = image->height;
    imageData = stack->stackData[(stack->base + stack->tot) % (IMAGE_STACK_MAX_SIZE + 1) + 1];
    (stack->tot)--;
